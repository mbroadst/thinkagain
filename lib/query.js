  ...
  */
  ...
  _executeCallback(fullOptions, parse, groupFormat) {
    if (this._error !== undefined) {
      return Promise.reject(new Errors.ThinkyError('The partial value is not valid, so the write was not executed. The original error was:\n' + this._error.message));
    }
  ...
  _validateQueryResult(result) {
    ...
    return Promise.all(promises)
      .then(_result => {
        if (!self._isPointWrite()) return _result;
        if (_result.length > 1) {
          throw new Errors.ThinkyError('A point write returned multiple values');
        }
        return _result[0];
      })
      .catch(error => {
        ...
        if (primaryKeys.length) {
          revertPromises.push(
            r.table(self._model.getTableName()).getAll(r.args(primaryKeys))
              .replace(doc => r.expr(keysToValues)(doc(self._model._pk)))
              .run()
          );
        }

        return Promise.all(revertPromises)
          .then(_result => { throw new Errors.ThinkyError('The write failed, and the changes were reverted.'); })
          .error(_error => {
            throw new Errors.ThinkyError('The write failed, and the attempt to revert the changes failed with the error:\n' + _error.message);
          });
      });
  }
  ...
  getJoin(modelToGet, _getAll, gotModel) {
    ...
    util.loopKeys(joins, (_joins, key) => {
      if (util.recurse(key, joins, modelToGet, getAll, gotModel)) {
        ...
        case 'hasMany':
          self._query = self._query.merge(doc => {
  ...
  addRelation(field, joinedDocument) {
    ...
    case 'hasMany':
      if (joinedDocument[joinedModel._pk] === undefined) {
        return new Query(model, self, {},
            new Errors.ThinkyError('Primary key for the joined document not found for a `hasOne/hasMany` relation.')
        );
      }
    ...
    case 'belongsTo':
      if (joinedDocument[joins[field].rightKey] === undefined) {
        if (joinedDocument[joinedModel._pk] === undefined) {
          return new Query(model, self, {},
              new Errors.ThinkyError('The primary key or the joined key must be defined in the joined document for a `belongsTo` relation.')
          );
        }
    ...
    case 'hasAndBelongsToMany':
      ...
      if (joinedDocument[joins[field].rightKey] === undefined) {
        if (joinedDocument[joinedModel._pk] === undefined) {
          return new Query(model, self, {},
              new Errors.ThinkyError('The primary key or the joined key must be defined in the joined document for a `hasAndBelongsToMany` relation.')
          );
        }
      ...
      if ((model.getTableName() === joinedModel.getTableName())
        ...
        linkValue = self._query(joins[field].leftKey)
        ...
        linkValue = self._query(joins[field].leftKey).do(leftKey => {
    ...
    default:
      return new Query(model, self, {},
            new Errors.ThinkyError('The provided field `' + field + '` does not store joined documents.')
        ).run();
    }
  }
  ...
  removeRelation(field, joinedDocument) {
    ...
    case 'hasOne':
      query = joinedModel
        .getAll(self._query(joins[field].leftKey), { index: joins[field].rightKey })
    ...
    case 'hasMany':
      ...
      return query;
    case 'belongsTo':
      query = self.replace(row => row.without(joins[field].leftKey));
      query.setPostValidation();
      return query;
    case 'hasAndBelongsToMany':
      let linkModel = joins[field].linkModel;
      if (joinedDocument === undefined) {
        query = self._query(joins[field].leftKey)
          .do(leftKey => {
        ...
        if (joinedDocument[joins[field].rightKey] === undefined) {
          ...
          if ((model.getTableName() === joinedModel.getTableName())
              && (joins[field].leftKey === joins[field].rightKey)) {
            query = self._query(joins[field].leftKey).do(leftKey => {
