...
class TypeString {
   ...
   * @throws {Error}
   */
  validate(str, prefix, options) {
    let _options = util.mergeOptions(this._options, options);
    ...
    if ((typeof str === 'function') && (str._query !== undefined)) {
      ...
      if ((this._enum !== undefined) && (this._enum[str] !== true)) {
        let validValues = Object.keys(this._enum);
        let message = 'The field ' + prefix + ' must be one of these values: ';

        for (let i = 0, ii = validValues.length; i < ii; ++i) {
          if (i === 10) { break; }
