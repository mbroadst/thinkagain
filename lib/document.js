...
class Document {
  ...
  */
   ...
   * @param {Function} reject The function to call if an error happened
   */
  _onSavedBelongsTo(copy, docToSave, belongsToKeysSaved, saveAll, savedModel, resolve, reject) {
    let self = this;
   ...
   * @param {Function} reject The function to call if an error happened
   */
  _saveSelf(copy, docToSave, belongsToKeysSaved, saveAll, savedModel, resolve, reject) {
    let self = this;
    let model = self._getModel();
    let constructor = self.__proto__.constructor; // eslint-disable-line
    let r = this._getModel()._thinky.r;
   ...
   * @param {Function} reject The function to call if an error happened
   */
  _onSaved(result, docToSave, saveAll, savedModel, resolve, reject) {
    ...
    util.tryCatch(() => { // Validate the doc, replace it, and tag it as saved
   ...
   * @param {Function} reject The function to call if an error happened
   */
  _saveMany(docToSave, saveAll, savedModel, resolve, reject) {
    let self = this;
    let model = self._getModel();
    ...
    util.loopKeys(model._joins, (joins, key) => {
      if (((key in docToSave) || (saveAll === true)) &&
        ...
        if (self[key] != null) { // eslint-disable-line
          ...
          let savePromise = self[key]._save(docToSave[key], saveAll, savedModel)
            .then(() => {
    ...
    util.loopKeys(model._joins, (joins, key) => {
      if (((key in docToSave) || (saveAll === true)) &&
        ...
        if (self.__proto__._hasMany[key] != null) { // eslint-disable-line
          for (let i = 0, ii = self.__proto__._hasMany[key].length; i < ii; ++i) { // eslint-disable-line
        ...
        for (let i = 0, ii = self[key].length; i < ii; ++i) {
          ...
          promisesMany.push(new Promise((resolve, reject) => { // eslint-disable-line
            if (!(self[key][i] instanceof Document)) {
              self[key][i] = new joins[key].model(self[key][i]); // eslint-disable-line
            }

            let callback = function() {
              self[key][i]._save(docToSave[key], saveAll, savedModel).then(doc => {
          ...
          for (let i = 0, ii = self[key].length; i < ii; ++i) {
            if (util.isPlainObject(self[key][i])) { // Save only if we have a full object, and not just a key
              promisesMany.push(new Promise((resolve, reject) => { // eslint-disable-line
                ...
                let callback = function() {
                  self[key][i]._save(docToSave[key], saveAll, savedModel).then(() => {
   ...
   * @param {Function} reject The function to call if an error happened
   */
  _saveLinks(docToSave, saveAll, resolve, reject) {
    let self = this;
    let model = self._getModel();
    let constructor = self.getModel();
  ...
  merge(obj) {
    ...
    util.loopKeys(obj, (_obj, key) => {
      ...
      if (util.isPlainObject(obj[key]) && util.isPlainObject(self[key])) {
        this.merge.call(self[key], obj[key]);
      } else {
        self[key] = obj[key];
      }
    });
