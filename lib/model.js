  ...
  */
  constructor(name, validate, options, thinkagain) {
    ...
    this._options.enforce_missing =
      (!!options.enforce_missing != null) ? options.enforce_missing : thinkagain._options.enforce_missing; // eslint-disable-line
    this._options.enforce_extra =
      (!!options.enforce_extra != null) ? options.enforce_extra : thinkagain._options.enforce_extra; // eslint-disable-line
    this._options.enforce_type =
      (options.enforce_type != null) ? options.enforce_type : thinkagain._options.enforce_type; // eslint-disable-line
  ...
  _parse(data, ungroup) {
    if (ungroup) {
      ...
      return this._parseArray(data)
        .error(err => {
          throw new Errors.ThinkAgainError('The results could not be converted to instances of `' + this.getTableName() + '`\nDetailed error: ' + err.message);
        });
    }
