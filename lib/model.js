  ...
  let p = new Promise(function(resolve, reject) {
    if (ungroup) {
      for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < data[i].reduction.length; j++) {
          util.tryCatch(function() {
          ...
          for (let i = 0; i < data.data.length; i++) {
            (function(i) { // eslint-disable-line
              ...
              if (Array.isArray(data.data[i][1])) {
                for (let j = 0; j < data.data[i][1].length; j++) {
...
Model.prototype.docRemoveListener = function(ev, listener) {
  if (Array.isArray(this._getModel()._listeners[ev])) {
    for (let i = 0; i < this._getModel()._listeners[ev].length; i++) {
