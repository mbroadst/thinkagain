...
class Model extends EventEmitter {
  ...
  */
  constructor(name, validate, options, thinky) {
    super();
  ...
  static new(name, schema, options, thinky) {
    let validate = thinky.ajv.compile(schema);
    let proto = new Model(name, validate, options, thinky);
  ...
  */
  hasOne(joinedModel, fieldDoc, leftKey, rightKey, options) {
    ...
    this._thinky.ajv.removeSchema(schema.id);
    documentModel._validate = this._thinky.ajv.compile(schema);
  ...
  */
  belongsTo(joinedModel, fieldDoc, leftKey, rightKey, options) {
    ...
    this._thinky.ajv.removeSchema(joinedModelSchema.id);
    joinedModelModel._validate = this._thinky.ajv.compile(joinedModelSchema);
  ...
  */
  ...
  _parseGrouped(data) {
    ...
    if (data === null) { // makeDocument is true, but we got `null`
      throw new Errors.ThinkyError('Cannot build a new instance of `' + this.getTableName() + '` with `null`.');
    }
  ...
  _parse(data, ungroup) {
    if (ungroup) {
      ...
      return this._parseArray(data)
        .error(err => {
          throw new Errors.ThinkyError('The results could not be converted to instances of `' + this.getTableName() + '`\nDetailed error: ' + err.message);
        });
    }
